                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card recent-sales overflow-auto">
                            ...
                            <div class="modal fade" id="affiliateStatusModal" tabindex="-1"
                                aria-labelledby="affiliateStatusModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                <label for="couponCode" class="form-label">Coupon Code (if
                                                    applicable)</label>
                                            ...
                                            <button type="button" class="btn btn-success"
                                                onclick="toggleAffiliateStatus()">Confirm</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="modal fade" id="sendFundsModal" tabindex="-1" aria-labelledby="sendFundsModalLabel"
                                aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <p><strong>Withdrawable Amount:</strong> <span id="withdrawableAmount"></span>
                                            </p>
                                            <div class="mb-3">
                                            ...
                                            <button type="button" class="btn btn-success" onclick="sendFunds()">Send
                                                Now</button>
                                            <div id="sendErrorMessage" class="text-danger mt-2" style="display: none;">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <table class="table datatable  table-hover table-sm mx-auto">
                                    ...
                                    <tbody>
                                        @foreach ($affiliate as $item)
                                            ...
                                            <div class="modal fade" id="affiliateStatusModal-{{ $item->id }}"
                                                ...
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <img src="{{ asset('uploads/' . $item->user->image) }}"
                                                                alt="User Image" class="img-fluid mb-3">
                                                            <p><strong>Name:</strong> <span>{{ $item->user->name }}</span>
                                                            </p>
                                                            <p><strong>CNIC:</strong> <span>{{ $item->user->cnic }}</span>
                                                            </p>
                                                            <p><strong>Address:</strong>
                                                                <span>{{ $item->user->address }}</span></p>
                                                            <p><strong>Phone Number:</strong>
                                                                <span>{{ $item->user->phone }}</span></p>
                                                            <p><strong>Email:</strong> <span>{{ $item->user->email }}</span>
                                                            </p>

                                                            <!-- Show Coupon Code and Percentage based on status -->
                                                            <div class="mb-3"
                                                                id="couponPercentageFields-{{ $item->id }}"
                                                                style="{{ $item->status == 1 ? 'display:none;' : '' }}">
                                                                <label for="couponCode-{{ $item->id }}"
                                                                    class="form-label">Coupon Code</label>
                                                                <input type="text" class="form-control"
                                                                    id="couponCode-{{ $item->id }}" name="couponCode"
                                                                    value="{{ $item->coupon }}" required>

                                                                <label for="percentage-{{ $item->id }}"
                                                                    class="form-label">Percentage</label>
                                                                <input type="number" class="form-control"
                                                                    id="percentage-{{ $item->id }}" name="percentage"
                                                                    value="{{ $item->percentage }}" required>

                                                                <label for="vendor_percentage-{{ $item->id }}"
                                                                    class="form-label">Vendor Percentage</label>
                                                                <input type="number" class="form-control"
                                                                    id="vendor_percentage-{{ $item->id }}"
                                                                    name="vendor_percentage"
                                                                    value="{{ $item->vendor_percentage }}" required>
                                                            </div>

                                                            <div id="errorMessage-{{ $item->id }}"
                                                                class="text-danger" style="display: none;"></div>
                                                            <!-- Error Message Display -->

                                                            <button type="button" class="btn btn-success"
                                                                onclick="approveAffiliate({{ $item->id }})">Confirm</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <tr class="text-center">
                                                ...
                                                <td>
                                                    <button type="button" class="btn btn-warning btn-sm"
                                                        ...
                                                        data-bs-target="#affiliateStatusModal-{{ $item->id }}">
                                                ...
                                                <td class="">
                                                    ...
                                                    <button class="btn btn-warning btn-sm"
                                                        onclick="openSendFundsModal({{ $item->id }}, {{ $item->amount }})">Send
                                                        Funds</button>
                                            ...
                                            <div class="modal fade" id="delete-{{ $item->id }}" tabindex="-1" aria-hidden="true" style="display: none;">
                                              <div class="modal-dialog modal-sm modal-dialog-centered">
                                                  <div class="modal-content">
                                                      ...
                                                      <form action="{{route('affiliate.delete',$item->id)}}" method="POST" class="delete-form">
                                                          @csrf
                                                          @method('DELETE')
